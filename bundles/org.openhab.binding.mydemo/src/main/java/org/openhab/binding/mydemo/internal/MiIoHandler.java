 ...
 */
package org.openhab.binding.mydemo.internal;

import static org.openhab.binding.mydemo.internal.MyXiaoMiBindingConstants.*;
...
import org.openhab.binding.mydemo.annotation.MessageListener;
import org.openhab.binding.mydemo.entity.Devices;
import org.openhab.binding.mydemo.entity.MiIoCommand;
import org.openhab.binding.mydemo.entity.json.MiIoSendCommand;
import org.openhab.binding.mydemo.internal.transport.Communication;
    ...
    protected synchronized Communication getConnection() {
        ...
        try {
            if (deviceId != null && deviceId.length() == 8 && tokenCheckPass(configuration.token)) {
                miioCom = new Communication(configuration.host, token, Utils.hexStringToByteArray(deviceId), lastId,
                ...
                if (miIOResponse != null) {
                    ...
                    Communication miioCom = new Communication(configuration.host, token, new byte[0], lastId,
                            configuration.timeout);
                    Message miIoResponse = miioCom.sendPing(configuration.host);
                    if (miIoResponse != null) {
